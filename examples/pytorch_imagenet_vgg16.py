...
parser.add_argument('--task-name', default='default',
                    help='the name of this running task')
parser.add_argument('--accuracy-threshold', type=int, default=93,
...
def train(epoch):
    ...
    with open('train_' + args.task_name, 'a+') as f:
        f.write('%s\t%d\t%f\t%f\t%f\t%f\n' % 
                ('train',
                epoch,
                train_loss.avg.item(),
                100. * train_accuracy_top1.avg.item(),
                100. * train_accuracy_top5.avg.item(),
                epoch_stop_time - epoch_start_time - 2,))
...
def validate(epoch):
    ...
    with open('validate_' + args.task_name, 'a+') as f:
        f.write('%s\t%d\t%f\t%f\t%f\n' %
                ('validate',
                 epoch,
                 val_loss.avg.item(),
                 100. * val_accuracy_top1.avg.item(),
                 100. * val_accuracy_top5.avg.item()))

    return val_accuracy_top5.avg
